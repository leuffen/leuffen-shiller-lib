...
5. **Zustand absichern:** ungespeicherte Änderungen/Entwürfe an betroffenen geladenen Documents führen zu UnsavedChangesException. Parallele externe Bearbeitung wird durch den Git-/Anwendungsablauf koordiniert. Eine eigene Revisionsprüfung der Adapter ist zunächst nicht erforderlich; die Quell-/Zielprüfungen und Behandlung tatsächlicher I/O-Fehler bleiben bestehen.
...
| Gleichzeitige externe Änderung | Externe Koordination im ersten Ausbau; Revisionsprüfung/ConflictException als späterer separater Testblock |
...
Zusätzliche spätere Abnahmefälle: zwei Kindanlagen in einem saveDocuments mit gemeinsamer Elternpromotion; transiente Elternübersetzung; Kategorieindex einer einzelnen Sprache löschen; verborgene Nachfahren beim Root-Delete; nach Move aktualisierter Editorzustand; No-op mit verweigerter Operation. Ein späterer Adapter mit Revisionsprüfung muss Vergleich und Mutation gegen Konkurrenz absichern. Diese Prüfung sowie ConflictException sind zunächst zurückgestellt und kein Grund, eine ansonsten unterstützte Mutation abzulehnen.

Jede gemeinsame Speicherung, auch mehrerer neuer Kinder, wird über genau einen Adapteraufruf write(list<Document>) ausgeführt. Der Adapter prüft die betroffenen Quell-/Zielzuordnungen und Rechte, plant Promotionen einmalig und aktualisiert nach Erfolg die internen Zustände. Originalkopien erhalten einen neuen Anlagezustand; sie übernehmen keine Revision oder andere technischen Metadaten des Originals. Öffentliche getTranslation/getTranslations werden dafür nicht im Adapter dupliziert.
